...
NVIDIA GB10 GPU          ← 121 GiB unified memory, Blackwell (SM 12.1)
                    ...
                    ┌─────────────────┐
Open WebUI ────────►  mcp-brave-search  (port 8765)
                    │  mcp-proxy wraps
...
| `mcp-brave-search` | 8765 | Brave Search MCP server (mcp-proxy → stdio bridge) | systemd |
...
| View Brave Search logs | `tail -f /var/log/mcp-proxy/mcp-brave-search.log` |
...
| Guide | Description |
|---|---|
| [`docs/prerequisites.md`](docs/prerequisites.md) | All required software — install this first. Covers apt packages, CUDA, Go, Docker, Miniforge, Node.js, mcp-proxy, and Hugging Face auth. |
...
| [`docs/brave-search.md`](docs/brave-search.md) | Set up Brave Search as an MCP tool server — mcp-proxy install, systemd service, Open WebUI registration, API usage examples. |
...
| `/var/log/mcp-proxy/` | Brave Search MCP proxy logs |
